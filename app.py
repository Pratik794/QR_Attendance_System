...
def gen_frames_attendance():
	...
	while True:
		...
		if decrypt:
			# for qr_code in decrypt:
			qr_code = decrypt[0]
			polygon_cords = qr_code.polygon
			img = gen.plot_polygon(img, polygon_cords)

			# Extract student details from QR code
			qr_data = qr_code.data.decode("utf-8")

			scanned_name, scanned_roll = qr_data.split(" ")
            ...
            # Validate if the student has already scanned today
			already_marked = any(row for row in attendance_records if len(row) >= 3 and row[0] == scanned_roll and row[1] == scanned_name and row[2] == today_date)

			if already_marked:
				print(f"⚠️ Attendance already marked for {scanned_name} ({scanned_roll}) today.")
			else:
                #Mark attendance
				with open(attendance_file, "a", newline="") as file:
					writer = csv.writer(file)
					writer.writerow([scanned_roll, scanned_name, today_date])
                
				print(f"✅ Attendance marked for {scanned_name} ({scanned_roll})")
